...
# __Q5.__ Dans la cellule ci-dessous, on simule une série de 10 000 lancers de dés. Déterminer sa moyenne et son écart type en utilisant les fonctions de la question **Q2**.
...
# __Q6.__ La fonction $frequence$ renvoie une liste contenant la fréquence d'apparition de chaque résultat, dans l'ordre croissant. Par exemple, [0.2, 0.25, 0.1, 0.15, 0.3, 0] signifie qu'il y a 20% de 1, 25% de 2, 10% de 3, 15% de 4, 30% de 5 et 0% de 6. Décrire le fonctionnement du code de cette fonction.
...
def simulationLancers(n):
    nbr_pile = 0
    for i in range(n):
        pass # Remplacer cette ligne par une instruction if permettant de décider si la pièce est tombée sur pile
        #    augmenter alors le nombre de piles de 1
    return nbr_pile/n
...
# 
# __Q9.__ Corriger la fonction ci-dessous permettant de simuler cette expérience, renvoyant le nombre de fois où Loic a touché la cible lors de ses 10 tirs.
...
# 
# __Q10.__ Corriger la fonction permettant de simuler cette expérience, renvoyant le nombre de fois où Loic a touché la cible en 10 tirs (4 lignes à corriger).
...
# 
# __Q13.__ Modifier la fonction $simulationFreq(n)$ renvoyant la fréquence d'apparition de chaque résultat, les résultats étant $\{Y=0\}$, $\{Y=1\}$, ..., $\{Y=10\}$.  
...
# Tester freq = simulationFreq(10000) une fois la fonction simululationFreq modifiée
# Tester afficheResultat(freq) une fois la fonction simululationFreq modifiée
...
def estime(n):
    c = 0
    # Écrire une boucle s'exécutant n fois
...
# 2\. On donne une fonction $echantillon(nb,n)$ simulant un échantillon de $nb$ familles à $n$ enfants et renvoyant une liste contenant le nombre de garçons dans chaque famille. Écrire une deuxième version de cette fonction en utilisant les listes en compréhension.
...
# 
# 3\. Compléter la fonction ci-dessous permettant de calculer la moyenne d'un échantillon de taille $nb$ de valeurs prises par la variable aléatoire $X$. Le résultat obtenu avec $n=10000$ vous semble-t-il cohérent ?
