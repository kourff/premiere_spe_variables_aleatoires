...
# __Q2.__ Compléter la fonction $moyenne(L)$ renvoyant la moyenne d'une liste de nombres $L$.
...
moy = moyenne(serie)
et = 0 # ligne à modifier pour calculer l'écart type de la série.
print("moyenne :", moy, "ecart-type: ",et)


# __Q6.__ Comprendre le fonctionnement du code suivant à l'aide des commentaires et des explications qui suivent.
...
def frequence(serie):
    ...
    for x in serie:                   # On parcourt la serie de nombres en comptant
        freq[x-1] = freq[x-1] + 1     #   le nombre d'apparitions de chaque valeur.
...
# 
# __Q7.__ Que renvoie l'expression $random()<0.5$ ? Tester dans une console Python si besoin. Expliquer comment l'utiliser pour simuler le lancer d'une pièce.

# __Q8.__ Compléter la fonction ci-dessous effectuant une simulation de $n$ lancers de pièces de monnaie et renvoyant la fréquence d'apparition de pile (ou face). On pourra la tester avec $n$ = 10 000 par exemple.

# In[1]:


def simulationLancers(n):
    # Écrire la ligne initialisant la variable nbr_pile comptant le nombre de piles
    for i in range(n):
        if random() < 0.5:
            # Écrire la ligne incrémentant de 1 le nombre de piles
    return nbr_pile/n

# Tester la fonction avec simulationLancers(10000)
...
# 
# __Q9.__ Corriger la fonction permettant de simuler cette expérience, renvoyant le nombre de fois où Loic a touché la cible lors de ses 10 tirs.
...
def simulation10TirV1():
    nbr_touche = 0
    for i in range(2):         # Corriger cette ligne
        if random() < 0.5:     # Corriger cette ligne
            nbr_touche += 1
    return nbr_touche
...
def simulation10TirV2():
    ...
    for i in range(9):
        if touche:
            proba = 0         # Corriger cette ligne     
        else:
            proba = 0         # Corriger cette ligne
        if random() < proba:
            nbr_touche += 0   # Corriger cette ligne
            touche = True
        else:
            touche = False
    return nbr_touche
...
# Tester avec print(simulationTirsV1(1000))
# Tester avec print(simulationTirsV2(1000))
...
def simulationDeplacements(n):
    ...
    for i in range(n):
        ...
        x = x + distance * np.cos(angle)  # calcul de la nouvelle valeur de x après le déplacement
        # ligne à écrire pour calculer la nouvelle valeur de y après le déplacement
    turtle.exitonclick()
    return x,y
...
# 
# __Q13.__ Compléter la fonction $simulationFreq(n)$ renvoyant la fréquence d'apparition de chaque résultat, les résultats étant $\{Y=0\}$, $\{Y=1\}$, ..., $\{Y=10\}$.  
...
def frequenceLettres(texte):
    ...
    for lettre in texmin:
        ...
        if 0 <= num <= 25:
            freq[num] += 1
    # Rajouter la ligne permettant de définir n
...
# 3\. Compléter la fonction $unLancer$ qui simule un lancer de deux dés et renvoie la valeur prise par $X$.  
# $|a-b|$ s'écrit abs(a-b) en Python.
...
# 4\. Corriger la fonction $moyenneEchantillon$ qui simule $n$ lancers de deux dés et renvoie la valeur moyenne de $X$ obtenue sur ces $n$ lancers. Le résultat obtenu avec 100 000 lancers est-il proche du résultat théorique ?
...
# 5\. Compléter la fonction suivante permettant d'estimer $P(X<3)$, la probabilité de l'évènement {$X<3$}. Vérifier que cette estimation correspond au résultat attendu.
...
def estime(n):
    c = 0
    for i in range(n):
        # Tester à chaque fois si le résultat d'un lancer X est inférieur à 3. Si c'est le cas:
        #      augmenter c de 1.
    return c/n
...
def famille(n):
    nbg = 0
    for i in range(n):
        if # compléter cette ligne avec une condition vérifiée avec une probabilité de 0.5
            # Compléter cette ligne pour incrémenter nbg si la condition est vérifiée
    return nbg


# 2\. On donne deux versions d'une fonction $echantillon(nb,n)$ simulant un échantillon de $nb$ familles à $n$ enfants. Quelles différence y a-t-il entre ces deux versions ? Que font-elles ?
...
# 
# 3\. Que renvoie la fonction ci-dessous ? On attend une interprétation précise. Le résultat obtenu avec $n=10000$ vous semble-t-il cohérent ?
